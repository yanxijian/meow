...
UINT16 MeowPinyin::GetPinyinIndexAll(const WCHAR * input, std::vector<unsigned short> * output, unsigned int offset, unsigned int length)
{
	...
	return static_cast<UINT16>(output->size());
}
...
bool MeowPinyin::RecursiveQuanpinPendingResult(unsigned int index, unsigned int offset_pinyin, PINYIN_RESULT pr) 
{
	...
	if (index == segments.size() - 1) 
	{
		...
		for (unsigned int i = 0; i < length; i++)
		{
			for (unsigned int j = 0; j < segment->solutions_quanpin_prefix[i].length; j++) 
			{
				// all full pinyin result
...
void MeowPinyin::ParseSegments()
{
	...
	{
		RecursiveQuanpinParser(&segment, segment.index, segment.length, 0);
	}
}
		...
		switch (status)
		{
			...
			case 1: // in pinyin
			{
				...
				else if (offset[i] == L'\n')
				{
					...
					{
						table_character[c_now].pinyin = static_cast<SHORT>(-2 - table_pinyin.size());
...
{
	return _py == py || BLOB_PINYIN_ALL_SHORTCUT[_py][0] == py || BLOB_PINYIN_ALL_SHORTCUT[_py][1] == py;
}
...
bool MeowPinyin::MatchCharToPinyin(WCHAR c, WORD py)
{
	...
	if ((table_character[c].pinyin >= 0) && MatchPinyinToPinyin(table_character[c].pinyin, py))
	{
		return true;
	}
	if (table_character[c].pinyin < -1)
	{
		...
		{
			if (MatchPinyinToPinyin(table_pinyin[offset + 1 + k], py))
			{
				return true;
			}
		}
